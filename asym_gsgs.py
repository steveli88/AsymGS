...
def entropy_toward_one(mask):
    """
    Encourages mask values to be close to 1 (not all zeros).
    Works with values in [0, 1] (e.g., after sigmoid).
    """
    eps = 1e-6
    mask = torch.clamp(mask, eps, 1 - eps)
    loss = -torch.log(mask)  # very large when mask → 0
    return loss.mean()
